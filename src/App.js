...
export default function App() {
  ...
  const handleResults = (selectedValue, choiceText) => {
    const url = `https://api.themoviedb.org/3/search/${selectedValue}?query=${encodeURIComponent(choiceText)}&include_adult=false&language=en-US&page=1`;
    fetch(url, options)
      .then((response) => response.json())
      .then((response) => setData(response.results))
      .catch((err) => console.error(err));
    ...
    <div className="App">
      ...
      { data.length
        ?(data[0].hasOwnProperty("known_for")
            ?data.map((person) => <Card_person key={person.id} person={person} />)
            :data.map((movie) => <Card key={movie.id} movie={movie} />)
          )
        :handleRecent()
      }
      <footer>Crée par Damien ETIENNE - Tout droit réservé</footer>
    </div>
  );
}
